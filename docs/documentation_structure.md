...
- `tools/import_project_bundle.py` – CLI companion for `ProjectImportService` that validates bundle checksums, copies manifests/patterns/assets into a working directory, and prints a JSON summary suitable for QA logs.
- `tools/autosave_stress_harness.py` – Synthetic workload generator that exercises the Tracker/Mixer autosave cadence, surfaces checkpoint/pruning metrics, and captures the recovery prompt text referenced in the QA hand-off pack.
...
- `docs/qa/autosave/README.md` – Autosave stress-harness evidence for the choir demo bundle, including the JSON summary exported by `tools/autosave_stress_harness.py` plus references to the generated `.autosave/` checkpoints stored alongside QA packs.
...
9. `poetry run python tools/mixer_trend_ci.py --baseline-json docs/qa/artifacts/mixer_trends/baseline/mixer_trend_baseline.json --output-json docs/qa/artifacts/mixer_trends/latest_summary.json --output-markdown docs/qa/artifacts/mixer_trends/latest_summary.md --history-json docs/qa/artifacts/mixer_trends/history/mixer_trend_history.json --history-markdown docs/qa/artifacts/mixer_trends/history/mixer_trend_history.md --sampler-manifest docs/assets/audio/sampler_s3_manifest.json --demo-automation` – Capture mixer trend deltas using the committed baseline, publish Markdown/JSON artifacts alongside the sampler manifest, and log SHA-256 digests for QA parity.
...
- 2025-11-28 – Documented the Step 8 import CLI, autosave stress harness, and QA hand-off references.
